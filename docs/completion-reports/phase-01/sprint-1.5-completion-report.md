...
## Sprint Summary
Sprint `1.5` hardened the Phase 1 fraud-evaluation slice into a submission-quality increment. The sprint added focused unit and integration coverage, OpenAPI polish, containerization, a real reviewer-facing `README`, and a final cleanup pass that improved request-error handling and aligned the documentation layer to the implemented system.
...
- The final reviewer-facing polish work uncovered real issues, not just cosmetic ones, especially around error handling and persistence fetch boundaries.
...
- Decide whether SpringDoc should become profile-specific after the take-home review path is no longer the primary audience.
...
| --- | --- | --- |
| `src/test/java/com/capitec/fraudengine/support/DomainTestFixtures.java` | Created | Shared domain-level fixture helper for unit tests |
| `src/test/java/com/capitec/fraudengine/domain/rule/impl/HighAmountFraudRuleTest.java` | Created | Unit tests for high amount thresholds |
| `src/test/java/com/capitec/fraudengine/domain/rule/impl/VelocityFraudRuleTest.java` | Created | Unit tests for velocity behavior and time-window semantics |
| `src/test/java/com/capitec/fraudengine/domain/rule/impl/RiskyMerchantCategoryFraudRuleTest.java` | Created | Unit tests for flagged merchant categories |
| `src/test/java/com/capitec/fraudengine/domain/rule/impl/UnusualTimeFraudRuleTest.java` | Created | Unit tests for overnight-window boundaries |
| `src/test/java/com/capitec/fraudengine/domain/policy/FraudDecisionPolicyTest.java` | Created | Unit tests for decision aggregation |
| `src/test/java/com/capitec/fraudengine/api/controller/FraudEvaluationControllerIntegrationTest.java` | Created, then modified | API integration tests plus OpenAPI and error-handling verification |
| `src/test/java/com/capitec/fraudengine/infrastructure/persistence/repository/FraudEvaluationJpaRepositoryIntegrationTest.java` | Created, then modified | Repository integration tests with Flyway-backed Postgres |
| `src/test/java/com/capitec/fraudengine/TestcontainersConfiguration.java` | Modified | Made public for reuse and pinned PostgreSQL image to `18.3` |
| `src/main/java/com/capitec/fraudengine/domain/rule/impl/HighAmountFraudRule.java` | Modified | Registered as Spring bean for integration path |
| `src/main/java/com/capitec/fraudengine/domain/rule/impl/VelocityFraudRule.java` | Modified | Registered as Spring bean for integration path |
| `src/main/java/com/capitec/fraudengine/domain/rule/impl/RiskyMerchantCategoryFraudRule.java` | Modified | Registered as Spring bean for integration path |
| `src/main/java/com/capitec/fraudengine/domain/rule/impl/UnusualTimeFraudRule.java` | Modified | Registered as Spring bean for integration path |
| `src/main/java/com/capitec/fraudengine/infrastructure/config/OpenApiConfiguration.java` | Created | OpenAPI metadata configuration |
| `src/main/java/com/capitec/fraudengine/infrastructure/config/FlywayConfiguration.java` | Created | Explicit Flyway bootstrap for app and tests |
| `src/main/java/com/capitec/fraudengine/api/controller/FraudEvaluationController.java` | Modified | Added OpenAPI documentation polish |
| `src/main/java/com/capitec/fraudengine/api/dto/FraudEvaluationRequestDto.java` | Modified | Added schema metadata |
| `src/main/java/com/capitec/fraudengine/api/dto/FraudEvaluationResponseDto.java` | Modified | Added schema metadata |
| `src/main/java/com/capitec/fraudengine/api/dto/FraudEvaluationSummaryResponseDto.java` | Modified | Added schema metadata |
| `src/main/java/com/capitec/fraudengine/api/dto/RuleResultResponseDto.java` | Modified | Added schema metadata |
| `src/main/java/com/capitec/fraudengine/api/dto/LocationDto.java` | Modified | Added schema metadata |
| `src/main/java/com/capitec/fraudengine/api/error/ApiErrorResponse.java` | Modified | Added schema metadata |
| `src/main/java/com/capitec/fraudengine/application/mapper/FraudEvaluationApplicationMapper.java` | Modified | Added explicit invalid-value handling |
| `src/main/java/com/capitec/fraudengine/api/error/GlobalExceptionHandler.java` | Modified | Added `400` handling for unsupported request-body values |
| `src/main/java/com/capitec/fraudengine/application/service/FraudEvaluationRetrievalService.java` | Modified | Added transactional boundaries for retrieval mapping |
| `src/main/java/com/capitec/fraudengine/common/error/InvalidRequestValueException.java` | Created | Shared invalid-request exception |
| `src/main/java/com/capitec/fraudengine/common/error/package-info.java` | Created | Common error package marker |
...
| `README.md` | Created, then modified | Reviewer-facing project documentation with final cleanup alignment |
...
## Close-Out
Sprint `1.5` is complete. The Phase 1 slice is now tested, documented, containerized, OpenAPI-enabled, and reviewer-ready. The repo also has a clear residual-debt list for any future post-submission hardening, but no remaining Sprint `1.5` tasks block close-out.
