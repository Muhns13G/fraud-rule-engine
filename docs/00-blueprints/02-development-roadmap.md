...
## Planning Principle
Move from a working fraud-decisioning slice to a configurable rules platform. Do not introduce dynamic rule authoring before the service can reliably accept categorized transaction events, evaluate a small static rule set, persist decisions, and expose retrieval APIs.

## Stage 1: Core Evaluation Slice
...
## Stage 2: Persistence And Auditability
...
- Keep infrastructure image versions explicitly pinned and reproducible.
...
## Locked-In Decisions
- The project is implemented as a categorized transaction fraud evaluation service, not a generic event platform.
...
- Security baseline uses profile-aware behavior: `default` open for local validation and `secure` with authenticated access controls.
