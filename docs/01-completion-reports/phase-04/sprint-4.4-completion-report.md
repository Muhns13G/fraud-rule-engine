...
- `4.4.1` Environment configuration templates and validation baselines:
  - added explicit env templates for `local-validation`, `secure`, and `production` operating modes
...
- `4.4.3` Phase 4 regression suite:
  - introduced canonical regression command: `scripts/run-security-ops-regression.sh`
...
- Keep default profile openness explicit and intentional for local validation ergonomics while hardening secure/production posture.
...
| --- | --- | --- |
| `docs/operations/env/local.env.template` | Created/Modified | Local/validation configuration baseline |
...
| `src/main/java/com/oitws/fraudengine/infrastructure/security/SecureProfileConfigurationGuardrails.java` | Modified | Fail-fast secure-profile guardrails |
| `src/test/java/com/oitws/fraudengine/api/controller/SecureProfileDatasourceResilienceIntegrationTest.java` | Modified/Created | Resilience-path coverage |
| `src/test/java/com/oitws/fraudengine/infrastructure/security/SecureProfileConfigurationGuardrailsTest.java` | Modified | Guardrail contract assertions |
| `scripts/run-security-ops-regression.sh` | Created | Canonical Phase 4 regression command |
