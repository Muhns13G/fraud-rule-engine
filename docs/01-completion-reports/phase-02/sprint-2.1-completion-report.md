...
- Updated `README`, RAG, and blueprint docs where the implemented persistence posture changed validation understanding
...
- Postgres `18+` container behavior is not a generic instability issue, but it does require the correct Compose mount layout for a smooth local hosted validation path.
...
- SpringDoc exposure warnings still appear because documentation endpoints remain enabled by default for validation usability.
...
- Consider whether `created_at` and `updated_at` should eventually be surfaced in a validation/admin-facing projection once Phase 2 retrieval scope broadens.
- Revisit startup-log cleanup for security and SpringDoc warnings once validation convenience is no longer the dominant concern.
...
| `src/main/java/com/oitws/fraudengine/infrastructure/persistence/entity/FraudEvaluationEntity.java` | Modified | Adds `createdAt`/`updatedAt` and JPA lifecycle auto-population |
| `src/test/java/com/oitws/fraudengine/infrastructure/persistence/repository/FraudEvaluationJpaRepositoryIntegrationTest.java` | Modified | Adds audit-persistence verification coverage |
| `src/test/java/com/oitws/fraudengine/TestcontainersConfiguration.java` | Modified | Keeps Testcontainers explicitly pinned to PostgreSQL `18.3` |
