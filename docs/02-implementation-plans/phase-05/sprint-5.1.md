...
## Scope
Production-hardening identity posture for non-local environments while preserving validation usability.
...
- profile posture:
  - `default`: open (local validation only)
...
- keep `default` profile open for local use with explicit guardrails
...
- Local validation ergonomics remain intact and explicit.
