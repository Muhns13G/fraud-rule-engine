...
## Scope
Start Phase 4 by hardening authentication and authorization boundaries for operations-facing surfaces while preserving explicit local/reviewer usability modes.
...
| `/actuator/health`, `/actuator/info`, `/actuator/metrics` | Open (current reviewer mode) | `OPS_READER` or stronger | Ops diagnostics surface |
| `/swagger-ui/**`, `/swagger-ui.html`, `/v3/api-docs/**` | Open (current reviewer mode) | Default disabled; when enabled require auth | Documentation surface remains profile-driven |
...
### Sprint 4.1.3
Harden default-profile safeguards without breaking reviewer ergonomics.
