...
PREFIX="local-validation-$(date +%Y%m%d%H%M%S)-${RANDOM}"
...
expect_status() {
  ...
  local actual
  actual=$(curl -sS -o /tmp/local_validation_response.json -w "%{http_code}" "$@")

  if [[ "$actual" != "$expected" ]]; then
    log "FAILED: ${description} (expected ${expected}, got ${actual})"
    cat /tmp/local_validation_response.json
    exit 1
  fi
...
preflight_status=$(curl -sS -o /tmp/local_validation_preflight_response.json -w "%{http_code}" "${BASE_URL}/actuator/info")
if [[ "${preflight_status}" != "401" ]]; then
  ...
  cat /tmp/local_validation_preflight_response.json
  exit 1
fi
